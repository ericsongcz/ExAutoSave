...
class UExAutoSaveBPLibrary : public UBlueprintFunctionLibrary
{
	...
	UFUNCTION(BlueprintCallable, meta = (DisplayName = "ExAutoSave", Keywords = "Auto Save GameInfo"), Category = "ExAutoSave")
		static void ExAutoSave(int setTimer, bool isDelaySave, FString gameSaveSlot, int gameSaveIndex, TArray<AActor*> gActor , TArray<FString> gString, TArray<FText> gText, TArray<FVector> gVector, TArray<FVector4> gVector4, TArray<FVector2D> gVector2, TArray<FRotator> gRotator, TArray<float> gFloat, TArray<int> gInt, TArray<bool> gBool, TArray<FTransform> gTransform, FExStructTemp ExStructArr, FExTypeStruct ExStructType, TArray<FExTypeStruct> ExStructTypeArr, bool&isSaved);
